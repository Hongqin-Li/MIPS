...
`define FORWARDE_FROM_EXECUTE 2'd2
...
module Controller(
    ...
    ControlUnit cu(Opcode, Func, CTL_D);
    assign PCSrc = Taken;

    //Pipeline Registers
    floprc #(`CTL_WIDTH_NUM) regE(CLK, RST, FlushE, CTL_D, CTL_E)
    
    flopr #(`CTL_WIDTH_NUM) regM(CLK, RST, 0, CTL_E, CTL_M)

    flopr #(`CTL_WIDTH_NUM) regW(CLK, RST, 0, CTL_M, CTL_W)
...
module Datapath(
    ...
    input [`CTL_WIDTH] CTL_D, CTL_E, CTL_W, CTL_M, 
    ...
    //DMEM
    output [31: 0] ALUOutM, MemWriteDataM,
    ...
    wire [`FORWARDE_WIDTH] ForwardAE = HCTL[`HCTL_FORWARDBE];


    //wire [31: 0] PCPlus4F, PCPlus4D, PCPlus4E, PCPlus4W;//For JAL and JALR

    //Fetch Stage
    wire [31: 0] PCNextF, PCF;
    ...
    //ForwardD
    assign ReadData1D = ForwardAD ? ALUOutM : ReadData1;
    assign ReadData2D = ForwardBD ? ALUOutM : ReadData2;
    ...
    wire BranchTaken
        = InstrD[31: 26] == `OP_BEQ ? ReadData1D == ReadData2D
        : InstrD[31: 26] == `OP_BNE ? ReadData1D != ReadData2D
        : InstrD[31: 26] == `OP_BGTZ ? ReadData1D >= ReadData2D
        : InstrD[31: 26] == `OP_BLEZ ? ReadData1D <= ReadData2D
        : 1'b0;
    ...
    assign [31: 0] PCTarget
        = CTL_D[`CTL_PCSRC] == `PCSRC_JUMPIMM ? {PCPlus4D[31:28], Instr[25: 0], 2'b0} 
        : CTL_D[`CTL_PCSRC] == `PCSRC_JUMPREG ? ReadData1
        : CTL_D[`CTL_PCSRC] == `PCSRC_BRANCH ? PCPlus4D + {signImmD[29:0], 2'b0} 
        : PCPlus4D;

    assign PCNextF = Taken ? PCTarget : PCPlus4F;
    //If Taken, Decode should be flushed
    assign FlushD = Taken;
    ...
    //Execute Stage
    wire [31: 0] ReadData1E, ReadData2E, ImmE, PCPlus4E;
    wire [4: 0] RsE, RtE, RdE, ShamtE;
    wire [31: 0] ALUOutE, MemWriteDataE;
    wire [4: 0] WriteRegE;

    floprc #(32) r1E(CLK, RST, FlushE, ReadData1, ReadData1E)
    floprc #(32) r2E(CLK, RST, FlushE, ReadData2, ReadData2E)
    ...
    wire [31: 0] ReadData1FWE 
        = ForwardAE == `FORWARDE_FROM_M ? ALUOutM 
        : ForwardAE == `FORWARDE_FROM_W ? ResultW
        : ReadData1E;

    wire [31: 0] ReadData2FWE 
        = ForwardBE == `FORWARDE_FROM_M ? ALUOutM 
        : ForwardBE == `FORWARDE_FROM_W ? ResultW
        : ReadData2E;
    ...
    assign MemWriteDataE = ReadData2FWE;
    ...
    //Memory Stage
    wire [31: 0] MemWriteDataM, ALUOutM, PCPlus4M;
    wire [4: 0] WriteRegM;
    flopr #(32) r1M(CLK, RST, MemWriteDataE, MemWriteDataM);
    flopr #(32) r2M(CLK, RST, ALUOutE, ALUOutM);
    flopr #(32) r3M(CLK, RST, WriteRegE, WriteRegM);
    flopr #(32) r4M(CLK, RST, PCPlus4E, PCPlus4M);

    //Writeback Stage
    wire [31: 0] ALUOutW, MemReadDataW, PCPlus4W;
    wire [4: 0] WriteRegW;
    flopr #(32) r1W(CLK, RST, ALUOutM, ALUOutW);
    flopr #(32) r2W(CLK, RST, MemReadDataM, MemReadDataW);
    flopr #(5)  r3W(CLK, RST, WriteRegM, WriteRegW);
    flopr #(32) r4W(CLK, RST, PCPlus4M, PCPlus4W);
    ...
    //Hazard Control
    wire MemToRegE = CTL_E[`CTL_REGWRITE_DATA] == `CTL_REGWRITE_DATA_FROM_MEMORY;
    wire MemToRegM = CTL_M[`CTL_REGWRITE_DATA] == `CTL_REGWRITE_DATA_FROM_MEMORY;
    wire BranchD = CTL_D[CTL_BRANCH] != 0;
...
module Hazard(
    ...
    input MemToRegE, MemToRegM, BranchD//If Decode is executing Branch
    output [`HCTL_WIDTH] HCTL);


    //Forwarding sources to D stage (For Branch)
    assign HCTL[`HCTL_FORWARDAD] = (RsD != 0 & RsD == WriteRegM & RegWriteM)
    assign HCTL[`HCTL_FORWARDBD] = (RtD != 0 & RtD == WriteRegM & RegWriteM)


    //Forwarding sources to E stage (ALU)
    assign HCTL[`HCTL_FORWARDAE]
        = RsE != 0 & RsE == WriteRegM & RegWriteM ? `FORWARDAE_FROM_M
        : RsE != 0 & RsE == WriteRegW & RegWriteW ? `FORWARDAE_FROM_W
        : 2'b0;
    assign HCTL[`HCTL_FORWARDBE]
        = RtE != 0 & RtE == WriteRegM & RegWriteM ? `FORWARDBE_FROM_M
        : RtE != 0 & RtE == WriteRegW & RegWriteW ? `FORWARDBE_FROM_W
        : 2'b0;
    ...
    assign HCLT[`HCTL_FLUSHF] = stallD;
